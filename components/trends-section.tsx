...
function SimpleLineChart({ data, metric }: { data: any[]; metric: string }) {
  const maxValue = Math.max(...data.map((d) => d[metric]))
  const minValue = Math.min(...data.map((d) => d[metric]))
  const range = maxValue - minValue

  return (
    <div className="h-48 flex items-end justify-between gap-2 px-4 py-4 bg-gray-50 rounded-lg">
      {data.map((point, index) => {
        const height = range > 0 ? ((point[metric] - minValue) / range) * 100 : 50
        return (
          <div key={point.month} className="flex flex-col items-center gap-2 flex-1">
            ...
            />
            <div className="text-xs text-gray-600">{point.month}</div>
          </div>
        )
      })}
    </div>
...
export function TrendsSection({ selectedFarm }: TrendsSectionProps) {
  ...
  const metrics = [
    ...
    { key: "rainfall", label: "Rainfall", unit: "mm", color: "text-cyan-600" },
  ]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">January Satellite View</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">June Satellite View</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="aspect-video bg-gradient-to-br from-green-200 to-green-400 rounded-lg flex items-center justify-center">
              <img
                src="/placeholder.svg?height=200&width=300"
                alt="June Satellite View"
                className="w-full h-full object-cover rounded-lg"
              />
            </div>
            <div className="mt-2 text-sm text-gray-600">
              Health: {trendsData[5].health}% | NDVI: {trendsData[5].ndvi}
            </div>
          </CardContent>
        </Card>
      </div>
